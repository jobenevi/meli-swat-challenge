...
#FUNCTIONS
## Utiliza o ConfigParser para ler as credenciais no arquivo cfg e utiliza para autenticar a request principal
def get_cred_data(config_name):
		cfg = configparser.ConfigParser()
		cfg.read(config_name)
		global appid	 
		global secret 
		appid = cfg['credenciais']['appid']
		secret = cfg['credenciais']['secret']

def autenticacao():
	return {'Authorization': 'Bearer {0}'.format(gera_token()['access_token'])}

def gera_token():
	global token
	if token == None:
		token = post_autenticacao()
	return token

def post_autenticacao():
	url = 'https://api.mercadolibre.com/oauth/token?grant_type=client_credentials&client_id={0}&client_secret={1}'.format(appid, secret)
...
def get_search_offset(site_id, seller_id, shipping, offset):
	...
	response = requests.get(url, headers=autenticacao())
...
def run():
    try:
        get_cred_data('credenciais.cfg')
    except:
	    print("Erro ao obter o token. Verifique as credenciais e o local do arquivo cfg")
